...
function sendTelegramMessage(text) {
  return new Promise((resolve, reject) => {
    const data = JSON.stringify({ chat_id: ADMIN_ID, text, parse_mode: 'Markdown' });
    const options = {
...
function sendTelegramPhoto(base64Data, caption) {
  return new Promise((resolve, reject) => {
    ...
    const partStart = [
      `--${boundary}`, `Content-Disposition: form-data; name="chat_id"`, '', ADMIN_ID,
      `--${boundary}`, `Content-Disposition: form-data; name="caption"`, '', caption,
      `--${boundary}`, `Content-Disposition: form-data; name="photo"; filename="image.jpg"`, `Content-Type: image/jpeg`, '', ''
    ].join('\r\n');
    const partEnd = `\r\n--${boundary}--\r\n`;
    const fullBody = Buffer.concat([Buffer.from(partStart), imageBuffer, Buffer.from(partEnd)]);
    const options = {
      hostname: 'api.telegram.org',
